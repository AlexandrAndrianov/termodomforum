		...
		********************************************************************/
		...
		<div class="forum-block raised-corners">
					<div class="forum-block-header">
						<h3><?=empty($arResult["GROUP"]["NAME"]) ? "Важное" : "Раздел"/*$arResult["GROUP"]["NAME"]*/?></h3>
					</div>
		...
		<?endif?>						
				...
				if (array_key_exists("FORUMS", $arRes))
				{
					...
					?>
					<?global $USER,$arPERM,$arRez;
						$arGrpUsr = $USER->GetUserGroupArray();
						$arRez = array_intersect($arGrpUsr, $arPERM);
					?>
			
					<li <?if(!empty($arRez)):?>ondblclick="defaultico(this);"<?endif?>>
								<div class="forum-icon">
								<?
									if(CModule::IncludeModule("iblock"))
									{ 
										$ibpen = new CIBlockPropertyEnum;
										$db_res = $ibpen->GetList(Array(), Array('PROPERTY_ID'=>LIST_ID,
																		'XML_ID'=>$res['ID']));
										$arField = $db_res->Fetch();
										
										if(empty($arField['XML_ID'])){
											/*Проверяем есть ли фото*/
											$db_res = $ibpen->GetList(Array(), Array('PROPERTY_ID'=>LIST_ID_IMG,
																		'XML_ID'=>$res['ID']));
											$arField = $db_res->Fetch();
								...
								?>
								<?if(empty($arField['XML_ID'])):?>	
									<i id="<?=$res['ID']?>" class="fa fa-files-o" <?if(!empty($arRez)):?>ondblclick="selectico(this, event);"<?endif?>></i>
								<?endif?>
								
								<?if($arField['PROPERTY_CODE'] === 'PROP_ICO'):?>
									<i id="<?=$res['ID']?>" class="<?=empty($arField['XML_ID'])? 'fa fa-files-o' : $arField['VALUE']?>" <?if(!empty($arRez)):?>ondblclick="selectico(this, event);"<?endif?>></i>
								<?endif?>
								
								<?if($arField['PROPERTY_CODE'] === 'PROP_IMG'):?>
									<img id="<?=$res['ID']?>" src="<?=$arField['VALUE']?>" <?if(!empty($arRez)):?>ondblclick="selectico(this, event);"<?endif?>></i>
								<?endif?>
		<?
						/*if ($res["NewMessage"] == "Y")
						{
			...
			<script>
				function selectico(el, event)
				{
					...
					'<div id="blockico">\
						...
						<input id="icofile" type="file" name="icofile" accept="image/*"/>\
...
<?if($_GET['key'] == 'icoselect'):?>
	...
	<?
				...
				{ 
					...
					/*Проверяем фото*/
					$db_resIMG = $ibpenum->GetList(Array(), Array('PROPERTY_ID'=>LIST_ID_IMG,
													'XML_ID'=>$_GET['id']));
					...
					/*Работаем с иконками*/
					$db_res = $ibpenum->GetList(Array(), Array('PROPERTY_ID'=>LIST_ID,
													'XML_ID'=>$_GET['id']));
					$arField = $db_res->Fetch();
		...
		<?else:?>
			...
			<? if(CModule::IncludeModule("iblock"))
						{ 
							...
							/*Проверяем иконки*/
							$db_resICO = $ibpenum->GetList(Array(), Array('PROPERTY_ID'=>LIST_ID,
															'XML_ID'=>$_POST['id']));
							...
							/*Работаем с фото*/
							$db_res = $ibpenum->GetList(Array(), Array('PROPERTY_ID'=>LIST_ID_IMG,
															'XML_ID'=>$_POST['id']));
