...
<?if($_GET['key'] != 'icoselect' || $_POST['key'] != 'fileico' ||
		...
		<?/*Функция сортировки массива с темами*/
			if(!function_exists('sortTwoArray')){

				function sortTwoArray(&$ar, $field, $lenar){
					...
					$ar = array_values($ar);//Переиндексация массива
					$cnt = count($ar);
					
					if($cnt < $lenar){
						/*Формируем массив для сортировки всех тем*/
						for($tm=0, $y=0; $tm < $cnt; $tm++){
							$cnttem = count($ar[$tm]);
							for($tmi=0; $tmi < $cnttem; $tmi++, $y++){
								$arC[$y][$y] = $ar[$tm][$tmi];
							}
						}
						...
						$cntsrt = count($arC);
						/*Сортируем массив*/
						for($ju=$cntsrt; $ju > 0; $ju--){
							for($bu=0; $bu < $ju; $bu++){
								$cur = strtotime($arC[$bu][$bu][$field]);
								$nex = strtotime($arC[intval($bu)+1][intval($bu)+1][$field]);
								if(!$cur){
									$cur = intval($arC[$bu][$bu][$field]);
									$nex = intval($arC[intval($bu)+1][intval($bu)+1][$field]);
								}
								if($cur < $nex){
									$zu = $arC[$bu][$bu];
									$arC[$bu][$bu] = $arC[intval($bu)+1][intval($bu)+1];
									$arC[intval($bu)+1][intval($bu)+1] = $zu;
								}
							}
						}
						/*Оставляем $lenar количество элементов*/
						for($tu=$lenar; $tu < $cntsrt; $tu++){
							unset($arC[$tu]);
						}
						...
						for($i=$cnt; $i > 0; $i--){
							for($u=0; $u < $i; $u++){
								$cur = strtotime($ar[$u][0][$field]);
								$nex = strtotime($ar[$u+1][0][$field]);
								if(!$cur){
									$cur = intval($ar[$u][0][$field]);
									$nex = intval($ar[$u+1][0][$field]);
								}
						...
						/*Удаляем темы которые ниже по релевантности*/
						for($r=0; $r < $cnt; $r++){
							$cntar = count($ar[$r]);
							for($t=1; $t < $cntar; $t++){
								unset($ar[$r][$t]);
						...
						/*Оставляем количество элементов равное $lenar*/
						for($g=$lenar; $g < $cnt; $g++){
							unset($ar[$g]);
						}
					}	
			...
			sortTwoArray($arResult['TOPICS'], 'START_DATE'/*'numMessages'*//*'VIEWS'*/, 5);
		?>
	...
	<?foreach($arResult["TOPICS"] as $key=>$val):?>
