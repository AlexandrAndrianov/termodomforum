...
class divaCTextParser
{
	...
	function convertText($text)
	{
		...
		{
			$divatxt = explode("\n", $text); 
			$divalen = count($divatxt);
			$str='';
			for($i = 0; $i < $divalen; $i++)
			{
				if(strripos($divatxt[$i], 'img') === false && strripos($divatxt[$i], 'div')===false && strripos($divatxt[$i], '#') === false)
				{
					if(ord($divatxt[$i]) != 13)
					{
						$divatxt[$i] = '<p>'.$divatxt[$i].'</p>';
					}
				}
				$str .= $divatxt[$i];
			}
	...
	function convert_video($params, $path)
	{
		...
		AddEventHandler("main", "TextParserVideoConvert", array("divaCTextParser", "TextParserConvertVideo"), 1000);
	...
	function convert_quote_tag($text = "")
	{
		...
		$text = str_replace('пишет:', 'написал:</span>', $text);
...
class divaforumTextParser extends divaCTextParser
{
	...
	function divaforumTextParser($lang = false, $pathToSmiles = '', $type=false, $mode = 'full')
	{
		$this->divaCTextParser();
	...
	{
		$text = divatextParser::convert4mail($text);
		if ($arFiles !== false)
			$this->arFiles = is_array($arFiles) ? $arFiles : array($arFiles);
		$this->arFilesIDParsed = array();
	...
	function convert_user($userId = 0, $name = "")
	{
		...
		if($userId > 0)
		{
			...
			return
				'<span><strong><a class="blog-p-user-name" id="bp_'.$anchor_id.'" href="'.CComponentEngine::MakePathFromTemplate($this->userPath,
					array(
						"user_id" => $userId,
						"USER_ID" => $userId,
						"uid" => $userId,
						"UID" => $userId)).'">'.$name.'</a></strong>'.
	...
	function convert_open_tag($marker = "quote")
	{
		...
		//return '<table class="forum-'.$marker.'"><thead><tr><th>'.($marker == "quote" ? GetMessage("FRM_QUOTE") : GetMessage("FRM_CODE")).'</th></tr></thead><tbody><tr><td>';
			return '<blockquote><div class="blockquote-inner">';
